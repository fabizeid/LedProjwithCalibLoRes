...
public class OPC implements Runnable
{
  ...
  void led(int index, int x, int y)  
  {
    ...
    pixelLocations[index] = x + width * y; //if(index == 1) println(width);
  }
  ...
  void ledStrip(int index, int count, float x, float y, float spacing, float angle, boolean reversed)
  {
    ...
    for (int i = 0; i < count; i++) {
	  ...
	  (int)round(x + (i - (count-1)/2.0) * spacing * c + 0.5),
        (int)round(y + (i - (count-1)/2.0) * spacing * s + 0.5));
    }
  }
               ...
               boolean flip)
  {
	...
	ledStrip(index + stripLength * (numStrips -1 -i), stripLength,
